...
# ── 润色 / 去 AI 味（走 AI，失败即显式报错）────────────────────
# 融合 humanizer-zh 核心规则：去填充词、破折号、排比、AI 高频词、三段式、金句，
# 强化「人」的呼吸感与口语节奏；同时守住国漫稿件的铁律（不增事实、保留占位符）。
POLISH_INSTRUCTION = """你是一位资深国漫自媒体编辑，请对下面的稿件做「去 AI 味」润色——只润色、不改选题、不增删事实。

【铁律】
1. 保留全部事实、数据、角色名、集数、平台名；禁止新增任何原文没有的事实或数字。
2. 保留原文中已有的全部【配图N：作品名_用途】占位符（N 与描述一字不改），位置可微调，不可删、不可改写；**原文没有占位符时，绝不要新增任何配图占位符**。
3. 标题与正文都不出现书名号《》。
4. 结尾保留或补一句互动引导（如「你觉得呢？」「评论区聊聊」）。
5. 直接输出润色后的正文全文，不要任何前言、解释、Markdown 代码块或「希望对你有帮助」这类客套话。
6. 保持原篇幅量级：只做润色，不要大幅扩写或缩写，篇幅与原文大致相当。

【去 AI 味清单——逐条自查并修正】
- 填充短语：删掉「为了实现这一目标」「值得注意的是」「在这个时间点」等绕弯子的话，直说。
- 破折号：把多余的「——」改成逗号或句号，一篇里最多留一处。
- 否定式排比：删掉「这不仅仅是…而是…」「不仅是…更是…」这类结构，平铺直叙。
- 三段式：别硬凑三个并列项（两项或四项都行）；列表项别用「**粗体词：** 说明」这种机械格式。
- AI 高频词：少用「此外、至关重要、深入探讨、彰显、交织、证明、凸显、活力、焕新、不可或缺、关键性」，能换就换。
- 系动词回避：该用「是/有」就直接用，别用「作为…的体现」「拥有…的特质」绕。
- 金句与升华：删掉像格言、总结陈词的话，留给读者自己感受。
