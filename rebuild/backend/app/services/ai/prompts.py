...
from .style_rules import STYLE_GUIDE, STRUCTURE_HINTS, DEPTH_GUIDE
...
- 小红书：情感/悬念型，短句分行，≤20字标题

## 🖼️ 配图
【配图N：作品名_场景描述】占位符，分散在各关键段落之间。**相邻两张图之间至少隔 2 段正文，绝不连续堆放、绝不堆在结尾**。编号从 1 连续递增、不重复。不少于 5 张。
...
5. 优先结合「近期真实国漫热点」：新番开播/定档、动画电影票房、角色或剧情争议、官方整活或名场面、行业大事件等——让选题踩在热点上，而不是纯架空策划。只要 user prompt 里提供了「近期真实国漫热点参考」（来自真实社媒讨论数据），**必须优先从中取材**，绝不用你训练记忆里的旧事件去替代它；只有参考为空时才退回到你确有把握的近期真实事件。
6. 【爆款四基因至少命中 2 条】：情绪钩子（共鸣/愤怒/爽感/治愈）、信息差（行业内幕/反常识/我知道你不知道）、身份标签（"这就是我""替我说出口"）、行动触发（实用清单/立刻能用）。标题不写震惊体，但必须有钩子与信息差，让人想点想转。
7. 选题覆盖与多样性：5 个至少覆盖 3 种 type。优先布局"昨日/今日一线动漫资讯与最新剧情"；其次小众动漫最新资讯/剧情；也要有一两个"动漫趣事 / 今日动漫人物生日或事迹 / 今日动漫大事记"；常青候选（沧元图/斗破/凡人等在播IP的已知动态、新番定档、国漫票房黑马等）可酌情纳入但不要占满，且**必须让位于近期真实热点**。避免与 avoid_titles / avoid_keys 已出现过的选题雷同。
8. 只输出一个 JSON 数组（务必带外层方括号 [ ]），元素为对象 {title,type,summary,angle,article_type,genes,why}，不要任何解释或 Markdown 代码块。"""

# 常青候选示例（日期无关，只作多样性锚点；真实近期热点优先于它们）
EVERGREEN_HINTS = "沧元图/斗破/凡人等在播IP的已知动态, 新番定档与开播, 国漫票房黑马, 经典IP二创热点"
...
def build_topic_prompt(
    ...
    if recent_context:
        parts.append(
            "【近期真实国漫热点参考】（来自小红书/抖音/公众号近30天真实讨论，按互动量排序；"
            "这是优先取材池，**必须优先从中挑选题**，禁止用你训练记忆里的旧事件替代它）：\n"
            + recent_context
        )
    else:
        parts.append(
            "（本次无外部近期热点数据。请只基于你确有把握的、当前正在发生/近期发生的国漫真实事件选材，"
            "宁写定性也不要编造；不要用过期旧事件充数。）"
        )
    parts.append(
        "优先从近 7 天真实发生的国漫热点事件中取材（新番、票房、争议、官方动作、名场面），"
        "让选题踩在热点上，而非纯架空。"
    )
    parts.append(f"常青候选参考（可写，但不必都写，且必须让位于近期真实热点）：{EVERGREEN_HINTS}")
    if avoid_titles:
