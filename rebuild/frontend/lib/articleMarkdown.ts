...
const IMG_BOX =
  "my-3 group relative";
const PH_BOX =
  "ph-slot my-2 flex cursor-default flex-col items-center gap-0.5 rounded-lg border-2 border-dashed border-[#d4a02c]/35 bg-[#fdf3d6]/60 px-4 py-3 text-center";
...
export function slotHtml(n: string, desc: string): string {
  ...
  return (
    ...
    `<div data-act="pick" class="mt-0.5 inline-flex cursor-pointer items-center gap-1 rounded-full border border-[#d4a02c]/40 px-3 py-0.5 text-[12px] font-medium text-[#b8862a] transition hover:border-[#c5942c] hover:bg-[#f5d06b]/20">📎 选图</div>` +
    `</div>`
  );
}
