...
export function TableHeader() {
  return (
    <div className="flex h-9 w-full items-center gap-4 px-4 text-xs text-tertiary">
      ...
      <span className={TABLE_COLS.action}>发布</span>
    </div>
  );
}

export function TableRow({ row }: { row: ArticleRow }) {
  const platform = PLATFORMS[row.platform];

  return (
    <div className="flex h-[61px] w-full items-center gap-4 rounded-row border border-subtle bg-card px-4 transition-colors hover:bg-raised">
